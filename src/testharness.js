 ...
 * The best way to get a list is to look in this file forfunctions names
    ...
    switch (typeof val) {
      case "string":
        for (i = 0; i < 32; i++) {
          ...
          val = val.replace(new RegExp(String.fromCharCode(i), "g"), replace);
        }
      ...
      case "object":
        ...
        if ("nodeType" in val &&
          ...
          case Node.ELEMENT_NODE:
            ...
            returnable = "Element node " + truncate(ret, 60);
            break;
          case Node.TEXT_NODE:
            returnable = 'Text node "' + truncate(val.data, 60) + '"';
            break;
          case Node.PROCESSING_INSTRUCTION_NODE:
            returnable = "ProcessingInstruction node with target " + format_value(truncate(val.target, 60)) + " and data " + format_value(truncate(val.data, 60));
            break;
          case Node.COMMENT_NODE:
            returnable = "Comment node <!--" + truncate(val.data, 60) + "-->";
            break;
          case Node.DOCUMENT_NODE:
            returnable = "Document node with " + val.childNodes.length + (val.childNodes.length === 1 ? " child" : " children");
            break;
          case Node.DOCUMENT_TYPE_NODE:
            returnable = "DocumentType node";
            break;
          case Node.DOCUMENT_FRAGMENT_NODE:
            returnable = "DocumentFragment node with " + val.childNodes.length + (val.childNodes.length === 1 ? " child" : " children");
            break;
          default:
            returnable = "Node object of unknown type";
            break;
          }
...
// See: http://www.w3c-test.org/resources/apisample.htm
